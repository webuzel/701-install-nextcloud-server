...
yaml_filename="701-install-nextcloud-server.yaml"
...
showhelp() {
  ...
  %s target_ip_address [admin_username]\n \
  где, \n \
...
showruncondition() {
    ...
    Имя пользователя: %s\n \
    Название файла с ярлыком: %s\n" \
		"${yaml_filename}" \
		"${bold}${desktop_ip}${normal}" \
		"${bold}${desktop_user}${normal}" \
		"${bold}${shortcut_file}${normal}"
...
echo "Теперь необходимо перейти на новый сайт по адресу http://${desktop_ip} и завершить установку Nextcloud."
# END OF SCRIPT
